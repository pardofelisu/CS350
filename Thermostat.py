...
class TemperatureMachine(StateMachine):
    ...
    def on_exit_cool(self):
        ...
        ## Turn off the blue light when we exit the cool state
        blueLight.off()
    ...
    def processTempIncButton(self):
        ...
        temperatureMachine.setPoint += 1
        temperatureMachine.updateLights()
    ...
    def updateLights(self):
        ...
        if(self.current_state.id == "heat"):
            if(temp < self.setPoint):
                ## If the temperature is below the setpoint, fade the red light
                redLight.pulse()
            else:
                ## If the temperature is above the setpoint, turn on the red light solid
                redLight.on()
        elif(self.current_state.id == "cool"):
            if(temp > self.setPoint):
                ## If the temperature is above the setpoint, fade the blue light
                blueLight.pulse()
            else:
                ## If the temperature is below the setpoint, turn on the blue light solid
                blueLight.on()
        elif(self.current_state.id == "off"):
            ## If the thermostat is off, turn off both lights
            redLight.off()
            blueLight.off()
    ...
    def setupSerialOutput(self):
        ## Setup the output string to send to the TemperatureServer
        output = f"{self.current_state.id},{floor(self.getFahrenheit())},{self.setPoint}\n"
        if(DEBUG):
            print(f"Output: {output}")
        ## Return the output string
        ser.write(output.encode('utf-8'))  # Encode the string to bytes before sending
        if(DEBUG):
            print(f"Sent to TemperatureServer: {output.strip()}")
        ## Return the output string 
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = f"{current_time:%Y-%m-%d %H:%M:%S}"
...
greenButton.when_pressed = tsm.cycleState
...
redButton.when_pressed = tsm.increaseSetPoint
...
blueButton.when_pressed = tsm.decreaseSetPoint
